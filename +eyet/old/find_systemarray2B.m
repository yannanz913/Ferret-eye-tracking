function [E,R,P] = find_systemarray2B(E_in, R_in, P_in, cMag, arrayscale)
...
%    Rin = [0; 0; 0];
%    Pin = [10;0;0];
%    [Eout,Rout,Pout] = eyet.find_systemarray2B(Ein,Rin,Pin,250,1);
...
GD_observed = [G_observed_1 G_observed_2 ]
...
X0 = [1;2.05;1;0;0;0;12.1;0;0];
Y = eyet.glints_systemarray2(X0,cMag,arrayscale)
size(Y)

[X,fval,exitflag,output] = fsolve(@(x) eyet.glints_systemarray2(x,cMag,arrayscale)-GD_observed, ...
	[ ...
        X0
    ],...
	options);
