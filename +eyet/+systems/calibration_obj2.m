function [world_points,marble] = calibration_obj2(varargin)
% CALIBRATION_OBJ - return real-world coordinates of the LEGO calibration object
%
% [world_points,marble] = eyet.calibration_obj(origin, l, d1, d2, h1, h2)
%
% THIS IS THE CALIBRATION OBJECT WITH MARBLE
% 
...
mark3 = [origin(1)-d1+d2; origin(2)-l/2; origin(3)+h1/2];
mark4 = [origin(1)-d1+d2; origin(2)+l/2; origin(3)+h1/2];
mark5 = [origin(1)-d1+d2; origin(2)-l/2; origin(3)+h1/2+h2];
mark6 = [origin(1)-d1+d2; origin(2)-l/2+0.15; origin(3)+h1/2+h2];

mark7 = [origin(1)-d1+d2-0.76/2+1.545/2; origin(2)+l/2+0.67; origin(3)+h1/2+h2+h1/2]; % middle point of the ball (on the surface)
